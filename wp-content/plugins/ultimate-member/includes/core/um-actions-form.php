			...
			if ( ! empty( $array['required'] ) ) {
				if ( ! isset( $args[ $key ] ) || $args[ $key ] == '' || $args[ $key ] == 'empty_file' ) {
					if ( empty( $array['label'] ) ) {
						UM()->form()->add_error( $key, __( 'Campo requerido', 'ultimate-member' ) );
					} else {
						UM()->form()->add_error( $key, sprintf( __( '%s es requerida', 'ultimate-member' ), $array['label'] ) );
					}
				}
			}
						...
						case 'unique_email':
							...
							if ( in_array( $key, array( 'user_email' ) ) ) {
								...
								if ( $args[ $key ] == '' && in_array( $key, array( 'user_email' ) ) ) {
									...
									UM()->form()->add_error( $key, __( 'Este no es un email valido', 'ultimate-member') );
								} elseif ( ! UM()->validation()->safe_username( $args[ $key ] ) ) {
